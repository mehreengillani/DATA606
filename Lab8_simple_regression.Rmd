...
Scatter plot to see the relationship between Personal Freedom and
Expression Control

```{r}
ggplot(hfi, aes(x = pf_expression_control, y = pf_score)) +
  geom_point(alpha = 0.7, color = "steelblue") +
  labs(x = "PF Expression Control (0 = most controls)",
       y = "Personal Freedom Score",
       title = "Relationship between Personal Freedom and Expression Control") +
  theme_minimal()
```
...
The high correlation of 0.796 strongly supports using linear regression
for prediction. Countries with greater media freedom (higher
pf_expression_control) reliably have higher overall personal freedom
scores.
...
The relationship appears to be roughly linear. The points generally
follow a straight-line pattern rather than a curve or other nonlinear
shape.

As PF expression control is 0 or around 0 there are very few data points
of personal frerdom score.

**Direction**: Positive relationship - As pf_expression_control
increases (indicating fewer political pressures and controls on media
content), the pf_score also increases. Countries with more media freedom
tend to have higher overall personal freedom scores.

**Strength**: Strong relationship - With a correlation of 0.796, the
points cluster fairly tightly around the imaginary line of best fit.
This indicates that knowing a country's expression control score gives
us good predictive power for estimating its personal freedom score.
...
variability in `pf_free` is explained by `pf_expression_control`.
...
Let's create a scatter plot with the least squares line for `m1` laid on
top.
...
6.  If someone saw the least squares regression line and not the actual
    ...
    **Insert your answer here**
    ...
    \
    for 6.7 pf_expression_control it seems like 7.9 pf_score on the line
    when we put it in the formula we get: $$
      \hat{y} = 4.61707 + 0.49143 \times pf\_expression\_control
    $$ 7.91

```{r}
# Find the country with pf_expression_control closest to 6.7
...
predicted <- 4.61707 + 0.49143 * country_6_7$pf_expression_control
residual <- country_6_7$pf_score - predicted

country_6_7$pf_expression_control
country_6_7$pf_score
residual

```

If someone saw only the least squares regression line, they would
predict a personal freedom score of approximately 7.9 for a country
with pf_expression_control = 6.7

This prediction is an OVERESTIMATE by approximately 0.503 points.

The negative residual of -0.503 indicates that:

The actual personal freedom score (7.43) is lower than what the regression line
predicts (7.9)
...
ggplot(data = m1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") + # changed line color to red
  xlab("Fitted values") +
  ylab("Residuals")
```
...
-   Pattern in Residuals Plot:

The residuals appear to be randomly scattered around the horizontal line
at y=0 with no strong systematic pattern. The points are fairly evenly
distributed above and below the zero line across the range of fitted
values.

**Evidence Supporting Linearity:**

-   Random scatter - No obvious curved pattern or systematic structure
-   Even distribution - Residuals are balanced above and below zero
    across all fitted values
-   No clear trends - The spread doesn't systematically increase or
    decrease
...
ggplot(data = m1, aes(sample = .resid)) +
  stat_qq()+
  stat_qq_line(color='red') # reference line has been added
```
...
**Insert your answer here** Based on the histogram and normal
probability plot, the nearly normal residuals condition appears to be
met.

The histogram shows that the majority of residual values are
concentrated within the range of -2 to 2, with a clear center around 0,
indicating a roughly symmetric, bell-shaped distribution.

The normal probability (Q-Q) plot further confirms this condition, as
most data points fall closely along the reference line, demonstrating
that the residuals follow an approximately normal distribution. This
linear pattern in the Q-Q plot satisfies the nearly normal residuals
assumption required for reliable linear regression inference.
...
Based on the residuals vs. fitted plot, the constant variability
condition appears to be reasonably met.

The spread of the residuals shows relatively consistent variability
across all ranges of the fitted values. There is no clear fanning
pattern where the spread systematically increases or decreases as the
fitted values change. The residuals maintain approximately the same
vertical spread throughout the horizontal axis, which satisfies the
constant variability assumption for reliable linear regression.
...
```{r}
colnames(hfi) # check columns

```

```{r}
# Create scatterplot with linear model
ggplot(hfi, aes(x = pf_religion, y = pf_expression)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(x = "Freedom of Religion Score", 
       y = "Freedom of Expression Score",
       title = "Relationship between Religious Freedom and Expression Freedom") +
  theme_minimal()

# Fit linear model
m_religion_expression <- lm(pf_expression ~ pf_religion, data = hfi)
summary(m_religion_expression)
```
...
```{r}
# Model 1: pf_expression_control predicting pf_score
m1 <- lm(pf_score ~ pf_expression_control, data = hfi)
summary(m1)$r.squared

# Model 2: pf_religion predicting pf_expression  
m_religion_expression <- lm(pf_expression ~ pf_religion, data = hfi)
summary(m_religion_expression)
```
...
Model 1 (pf_expression_control → pf_score): R² = 0.634 (63.4%) Model 2
(pf_religion → pf_expression): R² = 0.224 (22.4%) Conclusion:

pf_expression_control predicts pf_score MUCH better than pf_religion
predicts pf_expression.

**Why the Big Difference:**

Stronger Direct Relationship: Expression control is a direct component
of the personal freedom score, while religious freedom and expression
freedom are more distantly related concepts Better Explanatory Power:

-   Model 1 explains 63.4% of the variation in personal freedom scores

-   Model 2 explains only 22.4% of the variation in expression freedom
    scores Conceptual Closeness: A country's controls on media content
    (pf_expression_control) are more directly tied to its overall
    personal freedom rating than its religious freedom policies are to
    its specific expression freedom score
...
**Insert your answer here** Economic Freedom vs. Personal Freedom

I'd examine the relationship between ef_score (economic freedom) and
pf_score (personal freedom).

```{r}
# Scatterplot of economic vs personal freedom
ggplot(hfi, aes(x = ef_score, y = pf_score)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", color = "red") +
  labs(x = "Economic Freedom Score",
       y = "Personal Freedom Score",
       title = "Economic Freedom vs Personal Freedom") +
  theme_minimal()

# Fit linear model
m_econ_personal <- lm(pf_score ~ ef_score, data = hfi)
summary(m_econ_personal)
```

```{r}
# Residuals vs Fitted
ggplot(m_econ_personal, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Residuals vs Fitted")

# Q-Q Plot for normality
ggplot(m_econ_personal, aes(sample = .resid)) +
  stat_qq() +
  stat_qq_line() +
  labs(title = "Q-Q Plot of Residuals")

# Histogram of residuals
ggplot(m_econ_personal, aes(x = .resid)) +
...
R² = 0.401 - Economic freedom explains 40.1% of the variation in
personal freedom scores This is moderately strong but weaker than the
expression control relationship (R² = 0.634)

**Why This Might Be Surprising:**

Not as Strong as Expected: While positive, the relationship (R² = 0.401)
is weaker than some might assume - economic freedom doesn't perfectly
predict personal freedom

Residual Range: Residuals from -4.64 to +2.98 show substantial variation
unexplained by economic freedom alone This reveals that while economic
and personal freedoms are related, they're not inseparable - some
countries prioritize one over the other, creating fascinating case
studies in governance approaches.
